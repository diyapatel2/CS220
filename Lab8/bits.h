...
#define BF_MASK(start, len) ((BIT_MASK(len)) << (((start)-(len) +1)))
#define BF_PREP(x, start, len) (((x)&BIT_MASK(len)) << (((start)-(len))+1))
#define BF_GET(y, start, len) (((BF_MASK(start, len)) &(y)) >> (((start) -(len) +1)))
#define BF_SET(y, x, start, len) ((BF_PREP(x,start, len)) | (y))
